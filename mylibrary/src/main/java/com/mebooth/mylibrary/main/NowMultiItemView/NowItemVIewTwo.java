...
public class NowItemVIewTwo implements ItemViewDelegate<GetNowJson.NowData.NowDataList> {
    ...
    public NowItemVIewTwo(Context context, String type, MultiItemTypeAdapter adapter, ArrayList<GetNowJson.NowData.NowDataList> list) {
    ...
    public NowItemVIewTwo(Context context) {
        this.context = context;
    }
    ...
    public boolean isForViewType(GetNowJson.NowData.NowDataList item, int position) {
        if (item.getTopic().getImages().size() == 2) {
            return true;
        } else {
            return false;
        }
