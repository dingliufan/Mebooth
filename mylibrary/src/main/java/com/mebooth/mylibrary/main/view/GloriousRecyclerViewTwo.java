...
public class GloriousRecyclerViewTwo extends RecyclerView {
    ...
    public GloriousRecyclerViewTwo(Context context) {
        this(context, null);
    }

    public GloriousRecyclerViewTwo(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GloriousRecyclerViewTwo(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        if (TextUtils.isEmpty(mLoadingMoreText))
            mLoadingMoreText = "加载更多";
        if (TextUtils.isEmpty(mLoadMoreFailedText))
            mLoadMoreFailedText = "加载失败，点击重新加载";
        if (TextUtils.isEmpty(mLoadAllCompleteText))
            mLoadAllCompleteText = "所有数据已加载完毕";
        a.recycle();
    }
    ...
    private class GloriousAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            if (viewType == ITEM_TYPE_HEADER) {
                ...
                viewType = mOriginalAdapter.getItemViewType(newPosition);
