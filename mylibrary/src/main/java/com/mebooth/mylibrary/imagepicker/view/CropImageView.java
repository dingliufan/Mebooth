package com.mebooth.mylibrary.imagepicker.view;
...
//                    bitmap.recycle();
    ...
    public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        if (null != croppedImage && null != finalOutputFormat) {
            new Thread() {
                @Override
                public void run() {
     ...
     * 将图片保存在本地
     */
    private void saveOutput(final Bitmap croppedImage, Bitmap.CompressFormat outputFormat, File saveFile) {
